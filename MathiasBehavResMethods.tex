...
%P\left\{Z=2\mid{}W=1\right\}=h=\Phi\left(\frac{d-\beta}{\sqrt{2}}\right)\textrm{.}
\end{eqnarray*}
%degree of asymmetry in the placement of the yardstick, $\zeta$, is therefore an appropriate measure of bias. The midpoint of the yardstick 

% To calculate this measure, $\zeta$, we require the conditional probability $P\left\{Z=1\mid{}W=0\right\}$, denoted $g$, which is given by
...
%To measure bias under this model, we must first find equations for $\alpha$ and $\beta$. Since $\beta$ is the same as $l$ under the regular 2AFC model,
%\begin{eqnarray*}
%\beta=-\frac{1}{\sqrt{2}}\left[\Phi^{-1}\left(h\right)+\Phi^{-1}\left(f\right)\right]\textrm{.}
%\end{eqnarray*}
...
\lstinputlisting{simulate_yn_regular.py}
\appendix
\label{app:b}
...
\lstinputlisting{simulate_2afc_regular.py}
