...
\end{eqnarray*}Combining and rearranging these equations reveals that the maximum-likelihood estimate of $d$ under the modified 2AFC model is given by the same equation as under the regular 2AFC model (Eq.~5). Under the model, the subject could be said to be unbiased if the yardstick were centered on the first observation. The degree of asymmetry in the placement of the yardstick, $\zeta$, is therefore an appropriate measure of bias.

is the distance from $\Psi_0$ to the center of the yardstick, $\frac{1}{2}\left(\alpha+\beta\right)$. To calculate this measure, $\zeta$, we require the conditional probability $P\left\{Z=1\mid{}W=0\right\}$, denoted $g$, which is given by
\begin{eqnarray*}
